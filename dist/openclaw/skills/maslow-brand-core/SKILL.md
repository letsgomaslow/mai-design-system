...
1. Read `../../src/brand-contract.md` completely. In Hermes, use `${HERMES_SKILL_DIR}/../../src/brand-contract.md`.
2. Use `../../src/tokens.json` for exact values, `../../src/policies.json` for messaging and evidence rules, and `../../src/logo-assets.json` for the complete approved logo catalog. In Hermes, use `${HERMES_SKILL_DIR}/../../src/...`.
3. Start recurring work from an approved template or generator. Do not restyle from memory.
4. Use only the seven immutable designer PNG masters in `../../assets/logos/`. In Hermes, use `${HERMES_SKILL_DIR}/../../assets/logos/`. Never redraw, recolor, trace, crop, trim, retype, or re-encode them.
