...
const backupParent = mkdtempSync(join(tmpdir(), "maslow-artifacts-"));
const previousArtifacts = join(backupParent, "artifacts");
cpSync(artifactsRoot, previousArtifacts, { recursive: true });
...
try {
  ...
  run(python, ["scripts/stabilize-artifacts.py", "--root", root, "--phase", "office"]);
  ...
  run(python, ["scripts/stabilize-artifacts.py", "--root", root, "--phase", "pdf", "--previous", previousArtifacts]);
  ...
  console.log(`Built and inspected native artifacts in ${basename(artifactsRoot)}/`);
} finally {
